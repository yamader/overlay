# Copyright 1999-2024 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

EAPI=8

# ebuild generated by hackport 0.8.5.1

CABAL_FEATURES="lib profile haddock hoogle hscolour test-suite"
inherit haskell-cabal

DESCRIPTION="/etc/os-release helpers"
HOMEPAGE="https://github.com/yaccz/os-release"

LICENSE="BSD"
SLOT="0/${PV}"
KEYWORDS="~amd64"
IUSE="devel"

RDEPEND=">=dev-haskell/aeson-1.4:=[profile?]
	>=dev-haskell/megaparsec-7.0.5:=[profile?]
	>=dev-haskell/safe-exceptions-0.1.7.0:=[profile?]
	>=dev-haskell/text-1.2:=[profile?]
	>=dev-haskell/unordered-containers-0.2.10.0:=[profile?]
	>=dev-lang/ghc-9.0.2:=
"
DEPEND="${RDEPEND}
	>=dev-haskell/cabal-3.4.1.0
	test? ( >=dev-haskell/hspec-2.7.1
		>=dev-haskell/hspec-megaparsec-1.1.0
		>=dev-haskell/pretty-simple-1.0.0.0
		>=dev-haskell/tasty-1.3
		>=dev-haskell/tasty-golden-2.3.4
		>=dev-haskell/tasty-hspec-1.1.5.1 )
"

src_configure() {
	haskell-cabal_src_configure \
		$(cabal_flag devel devel)
}
