...
SLOT="0/${PV}"
KEYWORDS="~amd64"
IUSE="cross low-memory +no-exe +system-libarchive"

RDEPEND=">=dev-haskell/composition-prelude-2.0.5.0:=[profile?]
	dev-haskell/dlist:=[profile?]
	>=dev-haskell/unix-compat-0.1.2.1:=[profile?]
	>=dev-lang/ghc-9.0.2:=
	!no-exe? ( >=dev-haskell/bzlib-0.4:=[profile?] <dev-haskell/bzlib-0.7:=[profile?]
			dev-haskell/lzma-static:=[profile?]
			>=dev-haskell/zlib-0.6.2.2:=[profile?] <dev-haskell/zlib-0.7:=[profile?] )
	system-libarchive? ( app-arch/libarchive )
"
DEPEND="${RDEPEND}
